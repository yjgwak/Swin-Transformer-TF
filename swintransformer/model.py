...
class WindowAttentionV2(tf.keras.layers.Layer):
    def __init__(self, dim, window_size, num_heads, qkv_bias=True, meta_network_hidden_features=256, attn_drop=0.,
                 proj_drop=0., prefix=''):
        super().__init__()
        self.dim = dim
        self.window_size = window_size
        ...
        self.meta_network = tf.keras.Sequential([
            tf.keras.layers.Dense(meta_network_hidden_features, activation='relu'),
    ...
    def call(self, x, mask=None):
        ...
        relative_position_bias = tf.transpose(relative_position_bias, perm=[1, 0])
...
class SwinTransformerV2Block(tf.keras.layers.Layer):
    def __init__(self, dim, input_resolution, num_heads, window_size=7, shift_size=0, mlp_ratio=4.,
                 qkv_bias=True, meta_network_hidden_features=256, drop=0., attn_drop=0., drop_path_prob=0.,
        ...
        self.norm1 = norm_layer(epsilon=1e-5, name=f'{self.prefix}/norm1')
        self.attn = WindowAttentionV2(dim, window_size=(self.window_size, self.window_size), num_heads=num_heads,
                                      qkv_bias=qkv_bias, meta_network_hidden_features=meta_network_hidden_features,
...
class PatchMerging(tf.keras.layers.Layer):
    ...
    # TODO: check compute_output_shape
...
class BasicLayerV2(tf.keras.layers.Layer):
    def __init__(self, dim, input_resolution, depth, num_heads, window_size,
                 mlp_ratio=4., qkv_bias=True, meta_network_hidden_features=256, drop=0., attn_drop=0.,
                 drop_path_prob=0., norm_layer=LayerNormalization, downsample=None, use_checkpoint=False, prefix=''):
        super().__init__()
        self.dim = dim
        self.input_resolution = input_resolution
        ...
        # build blocks
        self.blocks = tf.keras.Sequential([SwinTransformerV2Block(dim=dim, input_resolution=input_resolution,
                                                                  num_heads=num_heads, window_size=window_size,
                                                                  shift_size=0 if (
                                                                          i % 2 == 0) else window_size // 2,
                                                                  mlp_ratio=mlp_ratio,
                                                                  qkv_bias=qkv_bias,
                                                                  meta_network_hidden_features=meta_network_hidden_features,
                                                                  drop=drop, attn_drop=attn_drop,
                                                                  drop_path_prob=drop_path_prob[i] if isinstance(
                                                                      drop_path_prob, list) else drop_path_prob,
                                                                  norm_layer=norm_layer,
...
class SwinTransformerV2Model(tf.keras.Model):
    def __init__(self, model_name='swin_tiny_patch4_window7_224', include_top=False,
                 img_size=(224, 224), patch_size=(4, 4), in_chans=3, num_classes=1000,
                 embed_dim=96, depths=[2, 2, 6, 2], num_heads=[3, 6, 12, 24],
                 window_size=7, mlp_ratio=4., qkv_bias=True, meta_network_hidden_features=256,
                 drop_rate=0., attn_drop_rate=0., drop_path_rate=0.1,
                 norm_layer=LayerNormalization, ape=False, patch_norm=True,
                 use_checkpoint=False, **kwargs):
        super().__init__(name=model_name)
        ...
        # build layers
        self.basic_layers = tf.keras.Sequential([BasicLayerV2(dim=int(embed_dim * 2 ** i_layer),
                                                              input_resolution=(patches_resolution[0] // (2 ** i_layer),
...
def SwinTransformerV2(model_name='swin_tiny_224', num_classes=1000, include_top=True, pretrained=True, use_tpu=False,
                      cfgs=CFGS):
    cfg = cfgs[model_name]
    net = SwinTransformerV2Model(
        model_name=model_name, include_top=include_top, num_classes=num_classes, img_size=cfg['input_size'],
        window_size=cfg[
            'window_size'], embed_dim=cfg['embed_dim'], depths=cfg['depths'], num_heads=cfg['num_heads']
    )
    net(tf.keras.Input(shape=(cfg['input_size'][0], cfg['input_size'][1], 3)))
    # TODO: pretrain weigts
    if pretrained is True:
        url = f'https://github.com/rishigami/Swin-Transformer-TF/releases/download/v0.1-tf-swin-weights/{model_name}.tgz'
        pretrained_ckpt = tf.keras.utils.get_file(
            model_name, url, untar=True)
